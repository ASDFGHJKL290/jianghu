...
class JianghuChat:
    ...
    def chat(self, user_input: str, history: list, npc_name: str, user_id: str = "default", combat_mode: bool = False) -> dict:
        ...
        if aq_list:
            task_lines = []
            for q in aq_list:
                qp = self.quest_progress.get(uid, {}).get(q["id"], {})
        ...
        print(f"====== [DEBUG] player_status injected:\n{player_status} ======\n")

        human_content = (
            f"{weather_str}{kb_text}\n{hist_text}\n{ltm_text}{gift_context}{player_status}\n{quest_text}\n\n"
            f"{step_guidance}\n【当前对话】\n玩家：{clean}"
        )
        ...
        # ── 调试：打印 LLM 原始输出的最后 500 字符 ──
        print(f"\n====== [DEBUG chat.py] raw (尾500): ...{raw[-500:] if len(raw)>500 else raw} ======\n")
        ...
        print(f"====== [DEBUG chat.py] _extract_gift result: {gift_info} ======\n")
        if gift_info:
            reply = self._strip_gift(reply)
            print(f"====== [DEBUG chat.py] gift extracted ✓, reply gift-stripped ======\n")
