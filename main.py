...
import os, uuid, time, asyncio, re, json, base64
...
async def api_chat_stream(req: g.ChatReq):
    """SSE 流式对话"""
    if g.chat is None:
        return JSONResponse({"reply": "系统正在初始化，请稍后刷新页面重试。"})
    ...
    async def gen():
        for chunk in g.chat.chat_stream(req.message, h, npc, uid):
            yield chunk
...
# ════════════════════════════════════════════════
# TTS/STT API（DashScope）
# ════════════════════════════════════════════════
import requests as _req

AUDIO_DIR = os.path.join(g.BASE_DIR, "audio")
os.makedirs(AUDIO_DIR, exist_ok=True)
TEMP_AUDIO_DIR = os.path.join(g.BASE_DIR, "temp_audio")
...
async def tts(npc_name: str = Form(default="店小二"), text: str = Form(...),
              emotion: str = Form("neutral"), speed: float = Form(1.0),
              pitch: float = Form(1.0), volume: float = Form(50)):
    if not DASHSCOPE_KEY:
        print("[TTS] 缺少 DASHSCOPE_API_KEY")
        return {"audio_url": None}
    try:
        # 使用 SSE 流式模式调用 DashScope TTS
        resp = _req.post(
            "https://dashscope.aliyuncs.com/api/v1/services/audio/tts/SpeechSynthesizer",
            headers={"Authorization": f"Bearer {DASHSCOPE_KEY}", "Content-Type": "application/json",
                      "X-DashScope-SSE": "enable"},
            json={"model": "cosyvoice-v3-flash",
                  "input": {"text": text, "voice": "longanyang", "format": "wav", "sample_rate": 24000}},
            stream=True,
            timeout=120
        )
        if resp.status_code != 200:
            print(f"[TTS] DashScope返回{resp.status_code}: {resp.text[:200]}")
            return {"audio_url": None}
        # 解析 SSE 流，收集音频 base64 数据
        audio_chunks = []
        line_count = 0
        for line in resp.iter_lines(decode_unicode=True):
            ...
            if line.startswith("data:"):
                ...
                try:
                    ...
                    # 记录前3条事件结构以便调试
                    if line_count <= 5:
                        print(f"[TTS] SSE#{line_count}: keys={list(evt.keys())} output_keys={list(evt.get('output',{}).keys())} has_audio={'audio' in evt.get('output',{})}")
                    # 尝试多种字段名
                    output = evt.get("output", {})
                    task_status = output.get("task_status", "")
                    audio_info = output.get("audio", {})
                    # 如果 audio 没有 data，尝试其他可能的路径
                    b64_data = audio_info.get("data", audio_info.get("audio_data", ""))
                    if b64_data:
                        audio_chunks.append(base64.b64decode(b64_data))
                    elif task_status == "succeeded" and not b64_data:
                        print(f"[TTS] SSE succeeded但无音频数据")
                    elif task_status == "failed":
                        err_msg = output.get("message", "未知错误")
                        print(f"[TTS] SSE任务失败: {err_msg}")
                        return {"audio_url": None}
                except json.JSONDecodeError as je:
                    if line_count <= 3:
                        print(f"[TTS] SSE#{line_count} JSON解析失败: {payload[:100]}")
        # 合并音频并保存
        if not audio_chunks:
            print(f"[TTS] SSE流中未收到音频数据")
            return {"audio_url": None}
        ...
        with open(audio_path, "wb") as f:
            f.write(audio_data)
        print(f"[TTS] 成功生成音频: {filename} ({len(audio_data)} bytes)")
...
async def get_audio(filename: str):
    ...
    media = "audio/wav" if safe_name.endswith(".wav") else "audio/mpeg"
    return FileResponse(audio_path, media_type=media)

@app.post("/api/stt")
async def stt(file: UploadFile = File(...)):
    if not DASHSCOPE_KEY:
        print("[STT] 缺少 DASHSCOPE_API_KEY")
        return {"text": ""}
    try:
        content = await file.read()
        print(f"[STT] 收到音频: {len(content)} bytes, type={file.content_type}")

        # 保存原始音频
        os.makedirs(TEMP_AUDIO_DIR, exist_ok=True)
        webm_path = os.path.join(TEMP_AUDIO_DIR, f"stt_{uuid.uuid4().hex}.webm")
        with open(webm_path, "wb") as f:
            f.write(content)

        # ffmpeg: webm → raw PCM (16kHz, mono, s16le)
        ...
        if ret.returncode != 0:
            print(f"[STT] ffmpeg转换失败: {ret.stderr[:200]}")
            try: os.remove(webm_path)
            except: pass
            return {"text": ""}
        print(f"[STT] 已转为PCM: {os.path.getsize(pcm_path)} bytes")

        # DashScope Recognition (实时语音识别)
        text = ""
        try:
            import dashscope
            ...
            class _DC:
                def on_complete(self, r): pass
                def on_error(self, r): pass
            rec = Recognition(model="paraformer-realtime-v2", callback=_DC(),
                             format="pcm", sample_rate=16000, language_hints=["zh"])
            result = rec.call(file=pcm_path)
            print(f"[STT] code={result.code}, msg={result.message}, output_keys={list((result.output or {}).keys())}")
            if result and not result.code:
                sentences = result.get_sentence()
                if sentences:
                    text = "".join(s.get("text", "") for s in sentences)
                    print(f"[STT] DashScope: '{text}'")
                else:
                    print(f"[STT] DashScope: 无句子 -> output={result.output}")
        except Exception as e:
            print(f"[STT] DashScope识别失败: {type(e).__name__}: {e}")

        # 清理临时文件
        try: os.remove(webm_path); os.remove(pcm_path)
        except: pass
