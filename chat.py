...
class JianghuChat:
    ...
    def chat(self, user_input: str, history: list, npc_name: str, user_id: str = "default", combat_mode: bool = False) -> dict:
        ...
        if chat_history:
            ...
            if len(chat_history) > recent_count:
                ...
                for t in older:
                    q = t.get("user", "").strip()
                    q_short = q[:30].rstrip("？。!！，, ")
                    if q_short and len(q_short) >= 2:
                        topics.append(q_short)
                # 去重 + 取最近20个
                seen = set(); unique = []
                for t in reversed(topics):
                    if t not in seen and len(unique) < 20:
                        seen.add(t); unique.append(t)
                if unique:
                    unique.reverse()
                    session_summary += "【之前聊过】" + "；".join(unique) + "。\n"
        ...
        # 情境触发：玩家消息中的关键词 → 相关记忆临时 +0.15 权重
        def _ngrams(s, n=2):
            return {s[i:i+n] for i in range(len(s)-n+1)}
        context_kws = _ngrams(clean, 2)
        for e in all_events:
            if not e.get("fixed") and any(kw in e.get("event", "") for kw in context_kws):
                e["_context_boost"] = 0.15

        fixed_mem = [e for e in all_events if e.get("fixed", False)]
        # 普通记忆按权重+情境加成排序
        normal_mem = sorted(
            [e for e in all_events if not e.get("fixed", False)],
            key=lambda e: e.get("weight", 0) + e.pop("_context_boost", 0),
            reverse=True
        )[:6]
        # 合并：固化排前面，普通按权重排后面
        ...
        # 解析 LLM 输出的 highlights → 每条写入经历层
        highlights = self._parse_highlights(raw)
        for h in highlights:
            if h and len(h) >= 2:
                self.memory.update_longterm(npc_name, h)
    ...
    def _parse_highlights(self, llm_raw: str) -> list:
        """从LLM输出中提取highlights列表"""
        if not llm_raw:
            return []
        # 匹配 {"highlights": ["话题1", "话题2"]}
        m = re.search(r'"highlights"\s*:\s*\[(.*?)\]', llm_raw, re.DOTALL)
        if not m:
            return []
        content = m.group(1)
        # 提取引号内的每个话题
    ...
    # ── 流式对话（SSE 推流）─────────────────────────────
    def chat_stream(self, user_input: str, history: list, npc_name: str,
                    user_id: str = "default"):
        """生成器：逐 token 推送 SSE，最后一帧携带完整元数据
        
        SSE 事件格式:
          data: {"type":"token","content":"字"}\n\n
          data: {"type":"done","reply":"...","emotion_state":{...},...}\n\n
        """
        profile = NPC_PROFILES.get(npc_name, NPC_PROFILES["店小二"])
        status, clean = sanitize_input(user_input)
        ...
        if status != "ok":
            reply = random.choice(profile["fallback_replies"][status])
            yield f"data: {json.dumps({'type':'done','reply':reply,'emotion_state':self.emotion_engine._default(),'actions':['idle'],'new_quests':[]},ensure_ascii=False)}\n\n"
            return

        # ── 复用 chat() 的 prompt 构建逻辑 ──
        # 知识库
        short_patterns = ["你好", "你好呀", "在吗", "哈哈", "嗯", "哦", "嗨", "再见", "说说", "聊聊"]
        kb_text = ""
        if len(clean) >= 8 and not any(p in clean for p in short_patterns):
            knowledge = self.search_knowledge(clean)
            kb_text = f"\n\n【江湖见闻】\n{knowledge}" if knowledge else ""
        weather_str = f"【今日天气】{self.weather['date']} {self.weather['type']}：{self.weather['desc']}"
        ...
        if chat_history:
            ...
            lines = []
            for t in recent:
                if t.get("user"): lines.append(f"玩家：{t['user']}")
                if t.get("assistant"): lines.append(f"你（{npc_name}）：{t['assistant']}")
            if lines:
                chat_hist_text = session_summary + "【对话历史】\n" + "\n".join(lines)
        ...
        normal_mem = sorted(
            [e for e in all_events if not e.get("fixed", False)],
            key=lambda e: e.get("weight", 0) + e.pop("_context_boost", 0), reverse=True)[:6]
        selected = fixed_mem + normal_mem
        mem_texts = [e["event"] for e in selected]
        mem_parts = []
        if fixed_mem:
        ...
        stream_llm = _LLM(
            api_key=self._ds_key, base_url=f"{self._ds_base}/v1",
            model="deepseek-v4-flash", temperature=0.6,
            request_timeout=60, max_tokens=400, streaming=True
        )
        ...
        # 亲密度
        intimacy = self.memory.get_relation("default", npc_name)
        rel_delta = RELATION_DELTA.get(emotion_state.get("primary", "neutral"), 0)
        new_relation = clamp(intimacy + rel_delta, 0, 100)
        self.memory.save_turn(npc_name, {
            "user": clean, "assistant": reply,
            "emotion": emotion_state.get("primary", "neutral"),
            "intensity": emotion_state.get("intensity", 0),
            "relation_delta": rel_delta, "timestamp": now_iso(),
        })
