  ...
  setup(){
    ...
    async function sendMessage(){
      ...
      try{
        ...
        if(d.realm){
          realm.value = d.realm;
          playerTitle.value = d.realm.title || realm.value.title;
        }

        // 突破提示
        if(d.breakthrough){
          triggerBreakthrough();
          showToast("🎉 突破成功!境界提升至「"+d.breakthrough.new_name+"·"+d.breakthrough.new_title+"」!", 5000);
          messages.value[npc].push({role:"system",content:"✨ 系统:恭喜!你已突破至「"+d.breakthrough.new_name+"·"+d.breakthrough.new_title+"」!"});
        }
        ...
        if(d.quest_step_completed&&d.quest_step_completed.length>0){
          d.quest_step_completed.forEach(sc=>{
            ...
            if(sc.next_hint){
              messages.value[npc].push({role:"system",content:"💡 "+sc.next_hint});
            }
          });
          await loadQuests();
        }
        if(d.quest_reward&&d.quest_reward.length>0){
          d.quest_reward.forEach(qr=>{
            let msg = "🎉 任务完成:"+qr.title+"\n奖励:"+qr.reward;
            if(qr.next_hint) msg += "\n\n📍 下一步指引:\n" + qr.next_hint;
            messages.value[npc].push({role:"system",content:msg});
            // 新手引导:初入江湖任务完成 → 推进到 Step 4(教学战斗)
            if(qr.title&&qr.title.includes("初入江湖")&&tutorialStep.value<4){
          ...
          await loadMapStatus(); // 任务奖励后刷新地图解锁

          // 章节进度推进
          if(d.chapter_advancement && d.chapter_advancement.advanced){
            chapterInfo.value = {
              chapter: d.chapter_advancement.new_chapter,
              title: d.chapter_advancement.chapter_title || "",
              advanced: true,
              completedAll: d.chapter_advancement.completed_all || false
            };
            showToast("📖 章节推进!第"+d.chapter_advancement.new_chapter+"章", 5000);
            messages.value[npc].push({role:"system",content:"📖 系统:章节已推进到第"+d.chapter_advancement.new_chapter+"章!"});
          }

          await loadQuests();
        }

        // 礼物接收(从 quest_reward 内部移出,独立触发弹窗)
        if(d.gift && d.gift.item){
          giftReceived.value = d.gift;
          showGiftModal.value = true;
          messages.value[npc].push({role:"system",content:"🎁 系统："+d.gift.from+" 赠送了你 "+d.gift.item + "！"});
          if(d.items)items.value=d.items;
          await loadMapStatus(); // 礼物后刷新地图解锁（可能含图碎）
        }

        // 结局触发
        if(d.outcome_info){
          outcomeInfo.value = d.outcome_info;
